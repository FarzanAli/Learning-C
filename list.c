...
Node *duplicate_list(Node *head){
  Node *newList = NULL;
  while(head != NULL){
    newList = insert_at_tail(newList, head->value);
    head = head->next;
  }
  return newList;
}
...
Node *efficient_delete_match(Node *head, int delete_value, int *num_deleted){
  ...
  *num_deleted = 0;
  ...
  Node *current1 = head;
  while(current1->value == delete_value && current1->next != NULL){
    current1 = delete_at_head(current1);
  ...
  Node *to_return = current1;
  
  while(current1->next->next != NULL){
    if(current1->next->value == delete_value){
      Node *current2 = current1->next;

      while(current2->value == delete_value){
        Node *delNode = current2;
