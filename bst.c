...
int main()
{
  ...
  simple_print(unbalance, 0);
  bstNode *balance = balanced_tree_copy(unbalance);
...
void traverseInOrder(bstNode *node, int *array, int *index){
  if (node == NULL)return;
  traverseInOrder(node->left_child, array, index);
  array[*index] = node->key;
  *index = *index + 1;
  traverseInOrder(node->right_child, array, index);
}

int *array_of_sorted_keys(bstNode *node){
  int *array = malloc(num_nodes(node) * sizeof(int));
  bstNode *headNode = node;
  int *index = calloc(1, sizeof(int));
  *index = 0;
  traverseInOrder(node, array, index);
  free(index);
  return array;
}
...
bstNode *construct_tree(int *array, int min, int max){
  if(min > max)return NULL;
    ...
    int median = floor((min + max)/2);
    bstNode *node = calloc(1, sizeof(bstNode));
    node->key = array[median];
    node->left_child = construct_tree(array, min, median - 1);
    node->right_child = construct_tree(array, median + 1, max);
  }
}

bstNode *balanced_tree_from_sorted_array(int *sorted_array, int length){
  bstNode *node = construct_tree(sorted_array, 0, length - 1);
  return node;
}

bstNode *balanced_tree_copy(bstNode *tree){
  int *array = calloc(num_nodes(tree), sizeof(int));
  array = array_of_sorted_keys(tree);
  bstNode *node = calloc(num_nodes(tree), sizeof(bstNode));
  node = balanced_tree_from_sorted_array(array, num_nodes(tree));
  free(array);
  return node;
}
